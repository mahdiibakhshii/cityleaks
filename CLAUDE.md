...
- **Monitor page** (`/monitor`): whole-map "contain" view over a downscaled overview image, reusing `PathLayer` + `NoteLayer` + live player dots. Connects as `role=monitor` — spawns no player and marks no cells.
...
- **Enemies (wandering NPCs):** server-authoritative entities that roam the city near players. Each keeps a **comfortable distance** from its nearest player — flees when crowded (`ENEMY_FLEE_RADIUS`), drifts back when abandoned (`ENEMY_LEASH_RADIUS`), orbits/wanders in between — and steers toward open streets (context steering) so it stays on roads and is hard to corner. The population **scales with player count** (`ENEMY_PER_PLAYERS`, clamped `ENEMY_MIN`..`ENEMY_MAX`); enemies spawn/despawn one-per-tick as players join/leave. Driven in the 10 Hz tick (`server/EnemyManager.ts`), broadcast like players (`ENEMY_*` events), rendered on every client as **Pac-Man-style ghost sprites** (`game/EnemyEntity.ts` + `game/EnemyManager.ts`, ghost body tinted by `EnemyDef.color`) and on the monitor (a second `DotManager`). **Ambient only today** — no contact effect; `ENEMY_TYPES` (shared/protocol) is the data-driven customization point (color/speed/radii/wanderiness per kind, parallel to `CHARACTERS`). Enemies stay on roads via a **server-side collision field** (see below) — the client mask was never visible to the server.
- **Production build** works (`npm run build` client, `npm start` server). Stress-tested: **140 bots, avg tick ~5 ms, peak ~41 ms, 0 errors** (< 50 ms budget).
...
**server/src/**
- `index.ts` — Express + Socket.IO bootstrap, static serving of `client/dist`, `/monitor` alias, `/api/status` (players, leak %, notes, enemies, `tickMs:{last,avg,max}`), graceful sync save of grid + notes on SIGINT/SIGTERM. Kicks off the (non-blocking) `CollisionField.build()` and constructs the `EnemyManager`.
- `GameServer.ts` — player `Map`, connection routing by `role` (game/monitor/td), 10 Hz `tick()` (broadcast positions, mark leak cells, send deltas, advance + broadcast enemies), tick-timing metrics.
...
- `CollisionField.ts` — server walkability: coarse (`COLLISION_GRID_SIZE`=2048) bit-packed WALKABLE grid over `MAP_BOUNDS`, built by decoding the mask PNG tiles (`pngjs`) and **cached to `server/data/collision.bin`** (loads instantly on restart; delete the file to rebuild after the mask changes). Exposes `isWalkable`/`isCircleWalkable`/`clearance`/`findNearestWalkable`/`moveWithSliding` (server ports of `CollisionMask.ts`) and a `ready` flag.
- `EnemyManager.ts` — wandering-NPC AI + lifecycle: context-steering toward open streets within a comfort distance band, population scaling, spawn/despawn. `update(players, dt)` returns spawn/despawn deltas; `getStates()`/`getPositions()` feed the `ENEMY_*` broadcasts.
...
- `TDRoom.ts` — TouchDesigner room (grid:full / delta / stats / reset). `config.ts` — re-exports + PORT, GRID_FILE, NOTES_FILE, COLLISION_FILE, `MASK_TILES_DIR` (dist→public). New server dep: **`pngjs`** (pure-JS PNG decode for the collision field).

**client/src/**
- `config.ts` — client-only constants: `ASSETS` (tile paths, `.webp` map ext, `OVERVIEW_PATH`), `MASK.ALPHA_THRESHOLD`, `STREAM`, `PLAYER`, `ENEMY` (render radius/z/lerp), `CAMERA`, `PATH` (path color), `MONITOR`.
- `config.ts` also has `SPRITE` (character/enemy sprite world sizes, anchor, walk FPS) + the `PATH` water palette.
- `game/Game.ts` — orchestrator + rAF loop (takes the chosen `characterId`). `Camera.ts` (fit-shorter-axis). `TileMap.ts`, `CollisionMask.ts` (streaming). `PathLayer.ts` (**shared pixel-art water leak overlay — also used by the monitor**). `NoteLayer.ts` (in-world note icons + proximity query, **also used by the monitor**) + `NoteUI.ts` (DOM button / compose modal / fullscreen reveal). `Player/RemotePlayer/PlayerManager` + `EnemyEntity.ts`/`EnemyManager.ts` (server-driven NPC ghosts) — all render via the sprite system below; `shapes.ts` is unused legacy.
- `game/sprites/` — the pixel-art sprite system. `drawSprite.ts` (THREE-free procedural pixel art: per-mascot `draw()` fns, auto outline, `renderSpec()` → canvas, the id/kind registry). `SpriteAtlas.ts` (bakes a spec → cached NearestFilter `BakedAtlas`; `playerAtlas(id)`/`enemyAtlas(color)`). `CharacterSprite.ts` (animated quad: 2-frame walk, L/R mirror, tint; `srgbTint()` helper).
...
- The **server** also reads these mask tiles once (via `pngjs`) to build its enemy collision field, caching the result to `server/data/collision.bin`. After re-splitting the mask, **delete `collision.bin`** so the server rebuilds it (otherwise it loads the stale cache; a `COLLISION_GRID_SIZE` change rebuilds automatically).
