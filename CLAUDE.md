...
- **Persistent data lives only on the server** at `/opt/cityleaks/server/data/` (leak grid, notes, kill markers, collision cache) — gitignored, untouched by deploys/reboots. Never commit it; never let a deploy delete it. All saves are **atomic** (`server/src/atomicWrite.ts`: temp file + rename) so a crash mid-write can't corrupt live data and a concurrent copy never sees a torn file.
- **Sync & back up player data** with `deploy/sync-data.sh` (PowerShell wrapper `deploy/sync-data.ps1`): `pull` (prod→local `server/data/`, snapshots local first), `backup` (timestamped `.tar.gz` of live prod into gitignored `backups/`), `restore <archive>` (guarded push back up: confirm → safety-backup → `pm2 stop`→push→`pm2 restart`), `list`. Syncs only the player files (`leak-grid.bin`, `notes.json`, `kills.json`); `collision.bin` is a regenerable cache, never synced. Uses `ssh`/`scp`+`tar` and the admin key `~/.ssh/cityleaks_hetzner` (env-overridable). See `deploy/README.md` → "Data sync & backups".
